...
HZZ4muAnalyzer::HZZ4muAnalyzer( const ParameterSet& pset )
  : fHist2muMass(0), fHist4muMass(0), fHistZZMass(0), fHist2muTime(0),
{
...
void HZZ4muAnalyzer::beginJob()
{
  ...
  fHist2mutime = fs->make<TH1D>(  "Hist2muTime", "2-mu Time[ns]", 100,  0., 100. ) ;
...
void HZZ4muAnalyzer::analyze( const Event& e, const EventSetup& )
{
          ...
          vit=Evt->vertices_begin(); vit!=Evt->vertices_end(); vit++ )
  {
      ...
      {
          if ( (*pout)->pdg_id() == 1000022 && (*pout)->status() == 2 ) 
          {	    
  ...
  if ( e.id().event() == 1 )
  {
   ...
   HepMC::FourVector part1Pos ;
   HepMC::FourVector part2Pos ;
   ...
   for ( unsigned int i=0; i<StableHiggsDesc.size(); i++ )
     {
       // skip other than mu
       //
       if ( abs(StableHiggsDesc[i]->pdg_id()) != 13 || abs(StableHiggsDesc[i]->pdg_id()) != 11) continue ; 
       
       for ( unsigned int j=i+1; j<StableHiggsDesc.size(); j++ )
         {
           // skip other than mu
           //
           if ( abs(StableHiggsDesc[j]->pdg_id()) != 13 || abs(StableHiggsDesc[j]->pdg_id()) != 11 ) continue ;
           //
	   ...
	   Mom2part = HepMC::FourVector((StableHiggsDesc[i]->momentum().px()+StableHiggsDesc[j]->momentum().px()),
           ...
           part1Pos = StableHiggsDesc[i]->end_vertext()->position();	   
           part1Pos = StableHiggsDesc[j]->end_vertext()->position();	   
           XMass2part = Mom2part.m() ;
           fHist2muMass->Fill( XMass2part ) ;
	   fHist2muTime->Fill( part1Pos.t() );
	   fHist2muTime->Fill( part2Pos.t() );
