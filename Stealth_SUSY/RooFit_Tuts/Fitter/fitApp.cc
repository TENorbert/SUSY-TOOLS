...
int main(int argc, char* argv[]) {
  ...
  TString dirpath("./DP_Analysis_inputfiles/");
  ...
  fgmsb.push_back( (TFile*)  TFile::Open(dirpath+"SPhoton_gmsb6000_gmsb6000.root"));
 // fvec.push_back( (TFile*) TFile::Open(dirpath+"PhotonJet_DATA_2011_ALL_pfakt5_CONTROL_QCD_2ndJet10_default.root")); //Data
  fvec.push_back( (TFile*) TFile::Open(dirpath+"SPhoton_C_DdataTime_C_DdataTime.root")); //Data
 // fvec_pj.push_back( (TFile*) TFile::Open(dirpath+"PhotonJet_DATA_2011_ALL_pfakt5_CONTROL_G_2ndJet10_default.root")); //Data
  fvec_pj.push_back( (TFile*) TFile::Open(dirpath+"SPhoton_C_DdataTime_C_DdataTime.root")); //Data
//  fvec_data.push_back( (TFile*) TFile::Open(dirpath+"PhotonJet_DATA_2011_ALL_pfakt5_2ndJet10_default.root")); //Data
  fvec_data.push_back( (TFile*) TFile::Open(dirpath+"SPhoton_C_DdataTime_C_DdataTime.root")); //Data
//  fvec_data.push_back( (TFile*) TFile::Open(dirpath+"Analysis_DPTrigger_Displaced_Photon_ALL_Jet-Sample.root")); //Data
 // fvec_ttbar = (TFile*) TFile::Open(dirpath+"PhotonJet_TT_2011_pfakt5_2ndJet10_default.root");  //MC
  fvec_ttbar = (TFile*) TFile::Open(dirpath+"SPhoton_C_DdataTime_C_DdataTime.root");  //MC
//  fvec_Wenu = (TFile*) TFile::Open(dirpath+"PhotonJet_WToENU_2011_pfakt5_2ndJet10_default.root");  // MC
  fvec_Wenu = (TFile*) TFile::Open(dirpath+"SPhoton_C_DdataTime_C_DdataTime.root");  // MC
  ...
  t = fit_2D( 0, 15401.1, CTAU, qcdscale, gjetscale ); // take values from command line
...
//--------------
 ...
 //  TString hfitname("BkgEst/my.phoTimeVsMet");
   TString hfitname_data("h_dataTimeVsMet");
   TString hfitname_Wenu("h_haloTimeVsMet");
   TString hfitname_gmsb("h_sgTimeVsMet");
   TString hfitname_pj("h_qcdTimeVsMet");
   TString hfitname_ttbar("h_spkTimeVsMet");
   TString hfitname_QCD("h_CSTimeVsMet");

  // TH2F* h2_data= (TH2F*)fvec_data[0]->Get(hfitname)->Clone("h2_data");
   TH2F* h2_data= (TH2F*)fvec_data[0]->Get(hfitname_data)->Clone("h2_data");
  ...
  //  TH2F* h2_Wenu_ = (TH2F*)fvec_Wenu->Get(hfitname);
    TH2F* h2_Wenu_ = (TH2F*)fvec_Wenu->Get(hfitname_Wenu);
    TH2F* h2_Wenu = (TH2F*)h2_Wenu_->Clone("h2_Wenu");
    protectZeroBin(h2_Wenu);

    h2_Wenu->Sumw2();
  //  h2_Wenu->Scale(lumin); //Halo data can't scale now!
 ...
 cout << "---Not Using dataset for QCD Control sample --- " << endl;

 cout << "--- dataset for QCD --- " << endl;

   // original histo
 //  TH2F* h2_QCD_ = (TH2F*)fvec[0]->Get(hfitname);
   TH2F* h2_QCD_ = (TH2F*)fvec[0]->Get(hfitname_QCD);
   ...
   TH2F* h2_gmsb_=(TH2F*) fgmsb[GMSBindex]->Get(hfitname_gmsb);
   ...
   // ttbar PDF
   TH2F* h2_ttbar_ = (TH2F*)fvec_ttbar->Get(hfitname_ttbar);
   TH2F* h2_ttbar = (TH2F*)h2_ttbar_->Clone("h2_ttbar");
   protectZeroBin(h2_ttbar);

   h2_ttbar->Sumw2();
//   h2_ttbar->Scale(lumin);  // Don't Scale Now as this is data
...
//   TH2F* h2_PJ_  = (TH2F*)fvec_pj[0]->Get(hfitname);
   TH2F* h2_PJ_  = (TH2F*)fvec_pj[0]->Get(hfitname_pj);
   ...
   printf("GMSB: %f   QCD: %f     PJ: %f\n",h2_gmsb->Integral(),h2_PJ->Integral());
   printf("DATA %f\n",h2_data->Integral());
...
//   h2_FakeMET->Add(h2_QCD);
 ...
 //  RooRealVar toy_met("toy_met", " GeV/c^{2}", 5., 8., 800.);
   RooRealVar toy_met("toy_met", " GeV/c^{2}", 5., 25., 800.);
  // RooRealVar toy_time("toy_time", " ns", 0.1, -2., 15.);
   RooRealVar toy_time("toy_time", " ns", 0.1, -5., 30.);
 ...
 //  xsection.setVal(h2_gmsb->Integral()/(lumin*efficiency[GMSBindex]));
   xsection.setVal(h2_gmsb->Integral()/(lumin*0.025));
